...
<div>
    <ul style="list-style-type: none; display: flex">
        ...
        <li style="margin-right: 20px;"><a href="/public/kinematik/gbb"><span class="text-success">Gleichförmig beschleunigte Bewegung</span></a></li>
...
<h2>Gleichmässig beschleunigte/verzögerte Bewegung</h2>
<div class="flex_wrapper">
    <div>Wenn die Geschwindigkeit nicht konstant ist und zunimmt, redet man von einer gleichmässig beschleunigten Bewegung, wenn die Geschwindigkeit abnimmt, redet man von einer gleichmässig verzögerten Bewegung.</div>
...
<div class="wrapper">
    ...
    <tr>
        <th>Berechnen</th>
        <td><button type="button" class="btn btn-success" onclick="GBB()">Berechnen</button></td>
    </tr>
    </table>
</div>
<div class="container">
    ...
    function GBB() {
        if(geschw_v.value && zeit_t.value && weg_s.value) {
           ...
           // console.log(math.evaluate(svt_formula, {v: geschw_v.value, t: zeit_t.value}));
            ...
            // console.log(math.evaluate(svt_formula, {v: geschw_v.value, t: zeit_t.value}));
            geschw_v.value = math.evaluate(bwz_formula, {t: zeit_t.value, s: weg_s.value});
            used_formula.innerHTML = '';
            used_formula.appendChild(mj(math.parse(bwz_formula).toTex({parenthesis: parenthesis})));
            var plot_fomula = bwz_formula.replace(/t/g, zeit_t.value);
            var plot_fomula = plot_fomula.replace(/s/g, weg_s.value);
